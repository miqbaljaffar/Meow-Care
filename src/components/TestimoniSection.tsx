...
const getTestimoni = cache(async () => {
  return prisma.testimoni.findMany({
    ...
    where: { status: 'PUBLISHED' }, // <-- HANYA TAMPILKAN YANG PUBLISHED
    include: {
      riwayatLayanan: { // <-- AMBIL DATA RELASI
        include: {
          kucing: {
            include: {
              pemilik: true,
    ...
    <section id="testimoni" className="py-20">
      <div className="container mx-auto text-center">
        ...
        {testimoni.length > 0 ? (
              ...
              <div key={item.id} className="bg-white p-8 rounded-lg shadow-lg text-left">
                ...
                <div className="flex items-center">
                  <div className="relative w-12 h-12 rounded-full bg-gray-200 mr-4 overflow-hidden">
                    <Image
                      src={'/kucing.jpg'} // Foto kucing belum ada di skema, pakai default
                      alt={`Kucing ${item.riwayatLayanan.kucing.nama}`}
                      layout="fill"
                      objectFit="cover"
                    />
                  </div>
          ...
          <p className="text-gray-500">Belum ada testimoni untuk ditampilkan.</p>
        )}
      </div>
    </section>
  );
