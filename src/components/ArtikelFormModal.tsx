      ...
      <div className="w-full max-w-2xl rounded-lg bg-white p-8 shadow-2xl my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label htmlFor="judul" className="block text-sm font-medium text-gray-800">Judul Artikel</label>
                <input id="judul" name="judul" type="text" defaultValue={artikelToEdit?.judul} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 focus:border-2" required />
            </div>
            <div>
              <label htmlFor="slug" className="block text-sm font-medium text-gray-800">Slug URL</label>
              <input id="slug" name="slug" type="text" defaultValue={artikelToEdit?.slug} placeholder="contoh: tips-merawat-kucing" className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 focus:border-2" required />
            </div>
          </div>

          <div>
            <label htmlFor="kutipan" className="block text-sm font-medium text-gray-800">Kutipan Singkat</label>
            <textarea id="kutipan" name="kutipan" rows={2} defaultValue={artikelToEdit?.kutipan} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 focus:border-2" required />
          </div>
          
          <div>
            <label htmlFor="konten" className="block text-sm font-medium text-gray-800">Konten (Markdown)</label>
            <textarea id="konten" name="konten" rows={8} defaultValue={artikelToEdit?.konten} className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 font-mono text-sm shadow-sm focus:border-cyan-500 focus:ring-cyan-500 focus:border-2" required />
            <p className="text-xs text-gray-600 mt-1">Anda bisa menggunakan sintaks Markdown untuk format teks.</p>
          </div>
